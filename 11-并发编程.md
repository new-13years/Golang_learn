...
![[attachments/Pasted image 20251230140208.png]]


**并发(concurrency)：指在同一时刻只能有一条指令执行，但多个进程指令被快速的轮换执行，使得在宏观上具有多个进程同时执行的效果，但在微观上并不是同时执行的，只是把时间分成若干段，使多个进程快速交替的执行。**

![[attachments/Pasted image 20251230140344.png]]

- 并行是两个队列**同时**使用两台咖啡机

- 并发是两个队列**交替**使用一台咖啡机
![[attachments/Pasted image 20251230140409.png]]
...
```
运行结果：
![[attachments/Pasted image 20251230140959.png]]
...
程序运行结果：
![[attachments/Pasted image 20251230141204.png]]
...
```
程序运行结果：
![[attachments/Pasted image 20251230142513.png]]
...
```
程序运行结果：
![[attachments/Pasted image 20251230144202.png]]
...
![[attachments/Pasted image 20251230144801.png]]
...
程序运行结果：
![[attachments/Pasted image 20251230150431.png]]
### 有缓冲的channel
...
示例图如下：
![[attachments/Pasted image 20251230150544 1.png]]
...
程序运行结果：
![[attachments/Pasted image 20251230150939.png]]
...
程序运行结果：
![[attachments/Pasted image 20251230151308.png]]
...
运行结果：
![[attachments/Pasted image 20251230151718.png]]
...
```
运行结果：
![[attachments/Pasted image 20251230154156.png]]
...
与switch语句可以选择任何可使用相等比较的条件相比， select有比较多的限制，其中最大的一条限制就是每个case语句里必须是一个IO操作，大致的结构如下：2
...
```
运行结果如下：
![[attachments/Pasted image 20251230155031.png]]
...
运行结果:
![[attachments/Pasted image 20251230155454.png]]
